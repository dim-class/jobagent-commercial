...
4. **相关基础**：JD 要求、而候选人自己动手练过（但还没在生产环境用过）
   的东西。**写成「有基础」「自己搭过」「上手过」，不要写「正在学」
   「在学习」「正在准备考试」**——后者等于告诉对方你还没有，把一个可用的
   筹码说成了短板。要具体到练的是什么（「自己搭过 K8s 集群」比
   「有 K8s 基础」有说服力），不要写「学习能力强」这种空话。

   **但不许升级成生产经验。**「自己搭过 K8s 集群」可以，「有 K8s 生产经验」
   不行——简历里没有的东西，换个说法也还是没有。这一条不因为好听而松动。
...
「有 X 基础」也必须有简历里的落脚点——简历写了在学、或者写了自己动手练过，
才能这么说；简历完全没提的技能，换任何说法都还是没有。
**措辞可以更有底气，事实不行。**「自己搭过 K8s 集群」和「有 K8s 生产经验」
之间隔着的是真假，不是语气。
