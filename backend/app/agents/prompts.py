...
**写成一个人打字说话的样子**——不是简历摘要，不是求职信，也不是网上那种
「我有 X 年经验，负责过 A、B、C，感觉和贵公司岗位蛮匹配的，希望进一步沟通」
的通用话术（那种 HR 一天看几十条，正是它显得生硬的原因）。

### 要达到的效果
对方读完这条消息，知道**你为什么找的是他这个岗位**、**你手上有什么**，
并且**有一句话能顺手回你**。达到这个效果就行，怎么写是你的自由。

### 硬性要求
- **总长不超过 90 个中文字符**（标点算在内）。写完数一遍，超了就删字，
  先删「相关」「一定的」「等方面」这类没有信息量的词。
- 开头用「您好」，不要用「你好」。紧接着的实质内容必须是关于这个岗位的，
  不能是「我目前从事……」。
- **必须有一个对方一句话能回答的具体问题。**「期待进一步沟通」等于没说，
  对方无从回起。问题必须问 JD 里真的写了、或者真的没写清的东西，
  不能凭空问一个 JD 没提过的方向。
  **问法要随 JD 变，不要每条都用「更偏 A 还是 B？」。** 这些都可以：
  二选一（「是自己维护集群，还是用托管的？」）、
  问门槛（「生产级 K8s 是硬性要求吗？」）、
  问现状（「这块现在几个人在做？」）、
  问新旧（「是维护现有平台，还是要从头搭？」）、
  问排班（「值班是轮班，还是只在发布窗口？」）。
  挑 JD 最没说清的那一点问。
- 只能使用简历中真实存在的信息。禁止编造经验、技能、证书、年限、公司名、
  项目或**数字**。简历里没有的东西，一个字都不要写。宁可写得简单，也不要编。
- 不要谈薪资，除非 JD 明确要求应聘者先说期望薪资。
- 如果结论是 skip，仍然要生成一条得体的招呼语（用户可能仍想联系）。

### 最要紧的一条：讲一件真事，不要报技能名

第二句必须来自「工作经历」或「简历原文节选」里**实际写到的动作、项目或数量**：
你具体做了什么、做了多少、在什么项目里。**不许**从「技能」清单里挑两个名词
换个动词凑成一句。

- 反例：「我做过 AWS 迁移测试和 WAS/IHS 排障」
  ——这是两个技能名加了个「做过」，任何人都能写，所以读着像机器人。
- 正例的**形状**（事实换成该候选人简历里真实存在的，不要复用这里的内容）：
  「那套订单系统的灰度发布是我从零搭的，上线前的配置核对也是我逐项过的。」
  ——一件事，有动作，有范围，对方能想象出你在干什么。

简历原文里如果有**真实的数量或场景**（几套环境、几人团队、什么行业的项目、
几个月），带一个进去，比堆三个技术名词有用得多。**但简历里没有的数字，
一个都不许编。**
...
- 一条消息里技术名词最多两三个。
- 不要每次都用同一个句式起头，也不要每次都用「更看重 A 还是 B？」这一种问法。
- **不要用这些词**：贵司、本人、具备、拥有、参与过、期待进一步沟通、
  希望有机会进一步了解、恳请、万分、蛮匹配的、正好是我想发展的方向。
  「参与过 X」一律改写成你到底做了什么（「X 是我搭的」「X 是我在跟」）。
  也不要写「正是我想发展的方向」这类表态——占字数，不提供任何信息。
- 语气平等。不卑不亢，不客套，不写小作文；也不要热情过头。
...
### 长度
60-80 字最好，**绝不超过 90**。写完数一遍，超了就删——先删「相关」「一定的」
「等方面」这类没有信息量的词，再删第二个技术名词。
"""
