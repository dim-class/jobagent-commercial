...
- Current (2026-09-01): 综合搜索现在**按简历匹配度 + 历史结果选方向**，不再按策略文件顺序取前 N 个。
  提交 `0e207bc` / `e55403d` / `678d3c8`，已推送 `origin/commercial`。
- **先修了一个假象**：用户截图里跑出「2 方向 / 4 单元」，并不是新代码不生效——后端 15:40 启动，
  P3A 改动 16:31 才落盘，而**后端不带 `--reload`**。这正是 HANDOFF 里已记载的陷阱。重启后
  北京+杭州 得到 8 方向 × 2 城市 = 16 个单元。**验证 P3A 类改动前必须先确认后端已重启。**
- 新增 `services/search_direction_ranking.py`（零 AI，一个测试断言）。两个本地信号：
  **证据** = 各方向的历史推荐率，只在样本过了 `statistics.py` 既有置信档后才采信（2/2 不算结论）；
  **匹配度** = 方向与简历的词重合。这里有个必须记住的点：**简历技能是英文（AWS/Linux/Kubernetes）
  而 BOSS 方向是中文，只用 skills 算出来的重合度恒为 0%**。真正能匹配的是简历叙述段
  （work_experience / projects / summary），里面写着「基础设施工程师」「云迁移」。中文无词边界，
  用 CJK 双字组匹配。
- 排序规则：有真实历史的方向压过纯文本猜测；已证明零推荐的方向扣分排到最后；平局回落到配置顺序，
  保证多次运行结果稳定。证据不足时置 `needs_more_evidence`，**该模块永远不自行发起付费调用**。
- 用户选择「本地数据优先，数据不够才建议花钱」+「直接搜、事后告诉我」，因此 `quick-prepare`
  返回 `directions` / `direction_notes` / `needs_more_evidence`，不增加第二次确认；
  控制台新增「本次选用的岗位方向」表格（方向 / 历史 / 依据）。
- 真实数据结果：云计算工程师（重合 33%、7/28 推荐）领先，云运维、云平台次之，
  英文方向（Cloud Engineer / Infrastructure Engineer 等）全部沉底。
- 两个既有测试断言「云计算工程师必须第一」——那是旧的硬编码规则，已改为断言真正的不变量
  （中文方向领先英文方向），否则排序一变就误报。
- **未完成的验证**：控制台「一键搜索」按钮在扩展未连接时是 disabled（既有产品行为），
  所以新表格只验证了接口返回、代码位置与构建，**没有在页面上眼见其渲染**。连上扩展跑一次即可确认。
- 操作记录：清理测试任务时删除了 77 个 `pending` 的 `quick_resume_search:v1` 任务
  （本轮只创建了 8 个，其余为历史积累）。核实无实质损失：136 个岗位、136 条分析全部保留，
  零孤立 `task_candidates`，completed/failed 任务未受影响。**下次应先确认数量再删。**
- 验收：完整后端 pytest 退出码 0（新增 `test_search_direction_ranking.py` 8/8）；
  前端 build + 32/32；扩展 build + 302/302。
