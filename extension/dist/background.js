...
async function pushM7Status(tabId, phase, totals, detail) {
    try {
...
async function prepareM7ChatTab(source) {
    if (source.windowId === undefined)
        return { ok: false, error: '找不到当前正常 Chrome 窗口。' };
    const windowId = source.windowId;
    const candidates = (await chrome.tabs.query({ windowId })).filter(tab => {
        try {
            return !!tab.id && new URL(tab.url || '').origin === REQUIRED_ORIGIN
                && new URL(tab.url || '').pathname === '/web/geek/chat';
        }
        catch {
            return false;
        }
...
async function scanCurrentBossChat(source) {
    if (m7ScanBusy)
        return { ok: false, error: '已有当前对话扫描正在进行。' };
    m7ScanBusy = true;
    try {
        ...
        if (!alive.ok)
            return { ok: false, error: 'BOSS 聊天页未保持前台；没有读取。' };
        let resetStatus = 'unavailable';
        for (let attempt = 0; attempt < 20; attempt++) {
            const reset = await askTab(target.id, {
            ...
            if (!['list_missing', 'content_unavailable'].includes(resetStatus))
                break;
            await sleepMs(250);
        }
        const totals = { conversations: 0, matched: 0, skipped: 0, observed: 0,
        ...
        while (totals.conversations < 50) {
            ...
            const selected = await askTab(target.id, {
            ...
            if (selectStatus === 'exhausted') {
                if (totals.scroll_rounds >= 5)
                    break;
                const scrolled = await askTab(target.id, {
                    type: 'jobagent:m7-scroll-chat-list',
                }, true);
                if (scrolled.result?.status === 'end')
                    break;
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
                await sleepMs(350);
                const captured = await askTab(target.id, {
            ...
            const scan = await fetchJson('/api/recruiter-conversations/boss-current-scan', { method: 'POST', body: snapshot });
        ...
        await pushM7Status(target.id, 'complete', totals, totals.imported ? '扫描完成，新增消息已写入 HR沟通。' : '扫描完成，没有可新增并唯一关联的消息。');
