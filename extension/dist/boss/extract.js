...
var BossExtract = (function () {
    ...
    function chatConversationMatches(doc, expectedCompany, expectedTitle) {
        ...
        // exists to stop.
        const listRoots = Array.from(doc.querySelectorAll(BossSelectors.CHAT_LIST.join(',')));
        //
        ...
        if (!listRoots.length)
            return { status: 'chat_list_unknown' };
        const scope = doc.body || pane;
        const nodes = Array.from(scope.querySelectorAll('*'))
            .filter((node) => !listRoots.some((list) => list === node || list.contains(node) || node.contains(list)));
        ...
        const found = (wanted) => values.some((value) => {
            ...
            for (let at = value.indexOf(wanted); at >= 0; at = value.indexOf(wanted, at + 1)) {
                ...
                if ((before === '' || BOUNDARY.test(before))
                    && (after === '' || BOUNDARY.test(after) || /\d/.test(after)))
                    return true;
            }
            return false;
        });
        ...
        // text is copied out.
        const present = (wanted) => values.some((value) => value.includes(wanted));
        ...
        const distance = (wanted) => {
            if (!doc.body || !(doc.body.textContent || '').includes(wanted))
                return -1;
            let node = pane;
