...
(function () {
    ...
    function renderRunnerBar(state) {
        removeRunnerBar();
        const bar = document.createElement('div');
        bar.id = RUNNER_BAR_ID;
        bar.setAttribute('style', 'position:fixed;top:64px;left:0;right:0;z-index:2147483645;' +
        ...
        pauseBtn.setAttribute('style', 'padding:3px 12px;border-radius:4px;border:1px solid #fff;background:transparent;' +
        ...
        resumeBtn.setAttribute('style', 'padding:3px 12px;border-radius:4px;border:1px solid #fff;background:transparent;' +
            'color:#fff;cursor:pointer;font:inherit;');
        ...
        cancelBtn.setAttribute('style', 'padding:3px 12px;border-radius:4px;border:1px solid #fff;background:transparent;' +
            'color:#fff;cursor:pointer;font:inherit;');
        ...
        if (state.pendingTerminal) {
            ...
            retryBtn.setAttribute('style', 'padding:3px 12px;border-radius:4px;border:1px solid #fff;background:transparent;' +
                'color:#fff;cursor:pointer;font:inherit;');
    ...
    function renderBar(session, onStop, onNext, onCapture, onScroll) {
        lastSession = session;
        removeBar();
        const bar = document.createElement('div');
        bar.id = BAR_ID;
    ...
    async function openNextCandidateStep() {
        ...
        if (next === -1) {
            await hardStop('no_candidates', '当前页面上没有更多可打开的候选人（可尝试"向下滚动"，不会自动查找），会话已结束。');
            return;
        }
        ...
        if (confirmed.session) {
            renderBar(confirmed.session, () => void stopFromBar(), () => void openNextCandidate(), () => void captureSelectedDetail(), () => void scrollOnePage());
        }
    ...
    async function scrollOnePageStep() {
        ...
        for (const c of page.candidates) {
            if (c.source_url)
                observedUrls.add(c.source_url);
        }
        const beforeCount = observedUrls.size;
        const after = BossContentScript.handle({ type: BossContentScript.DETECT });
        if (!after.ok || !after.result) {
            ...
            setStatusLine('已滚动一步，但读取滚动后的页面失败，无法统计候选人数量（不会自动重试），可再次点击"向下滚动"。');
            setScrollButtonDisabled(false);
            return;
        }
    ...
    async function init() {
        ...
        const snapshot = await askBackground({ type: 'jobagent:session-snapshot' });
        if (snapshot.kind === 'session') {
            renderBar(snapshot.session, () => void stopFromBar(), () => void openNextCandidate(), () => void captureSelectedDetail(), () => void scrollOnePage());
        }
        ...
        const runnerStatus = await askBackground({
            type: 'jobagent:runner-status',
        });
        if (runnerStatus.ok && runnerStatus.runner)
            renderRunnerBar(runnerStatus.runner);
    }
    ...
    if (!globals[flag]) {
        ...
        chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
            ...
            if (msg.type === 'jobagent:session-started' && msg.session) {
                ...
                renderBar(session, () => void stopFromBar(), () => void openNextCandidate(), () => void captureSelectedDetail(), () => void scrollOnePage());
