...
function salaryFailureCategory(reason: string): string {
  ...
  if (text.includes('frame_unavailable')) return 'salary_frame_no_reply'
  if (text.includes('abort')) return 'ocr_aborted'
  if (text.includes('failed to fetch') || text.includes('backend')) return 'backend_unreachable'
  return 'unavailable'
}
...
async function startRunner(taskId: number, candidateCap: unknown, binding?: PopupBinding,
  ...
  if (!validCandidateCap(candidateCap)) return { ok: false, error: `候选上限必须是 1–${RUNNER_MAX_TARGET} 的整数，请重新确认。` }
...
async function startBatch(
  ...
  if (!validCandidateCap(candidateCap)) return { ok: false, error: `候选上限必须是 1–${RUNNER_MAX_TARGET} 的整数。` }
...
async function executeM6Application(
  ...
  try {
    ...
    if (!preflight.ok || !preflight.result || preflight.result.status !== 'ok'
      ...
      if (reason === 'posting_closed') {
        // BOSS says this one is gone, so retire it here rather than leaving it
        // in the queue to be confirmed again tomorrow. Skipping is reversible
      ...
      for (let attempt = 0; attempt < M6_COMPOSER_ATTEMPTS; attempt++) {
        ...
        if (status === 'typed') {
          for (let send = 0; send < M6_SEND_ATTEMPTS; send += 1) {
            ...
            const alive = await verifyRunnerTab(tabId, true)
            if (!alive.ok) { status = 'foreground_lost'; break }
            const sent = await askTab<{ status: string }>(tabId, {
        ...
        if (status !== 'no_composer' && !status.startsWith('greeting_unavailable:')) break
...
async function readExperienceFilter(
  source: chrome.tabs.Tab,
): Promise<{ ok: boolean; error?: string; options?: { label: string; code: string }[] }> {
  if (activeRunToken !== null || salaryOcrBusy) {
  ...
  const response = await askTab<{ options?: { label: string; code: string }[]
    labels_without_code?: number; reason?: string | null }>(
    tab.id, { type: 'jobagent:read-experience-filter' }, true)
  if (!response.ok || !response.result) return { ok: false, error: '页面未就绪，请刷新 BOSS 搜索页后重试。' }
  const options = response.result.options ?? []
  if (!options.length) {
    return { ok: false, error: response.result.reason === 'not_a_search_page'
...
async function consoleCommand(message: unknown, sender: chrome.runtime.MessageSender): Promise<unknown> {
  ...
  if (data.action === 'read-experience-filter') return readExperienceFilter(source)
