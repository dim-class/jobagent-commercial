...
//: A domain error's `detail` payload (e.g. `{"field": "candidate_cap", ...}`)
//: carries *why* a 4xx happened - `core/errors.AppError`'s shape across the
//: whole backend. Attached to the thrown `Error` as `.detail` so a caller
//: that needs to distinguish "denied for a specific, known reason" from
//: "failed for some other reason" can, without parsing the Chinese message
//: text (CLAUDE.md M4f review item 3 - "distinguish denial/error").
...
async function fetchJson<T>(path: string, init?: { method?: string; body?: unknown }): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const err: FetchJsonError = new Error(message || `HTTP ${response.status}`)
    err.detail = (payload as { detail?: unknown } | null)?.detail
    throw err
  }
  return payload as T
}
...
interface NavigateResult {
  ...
  session?: SessionOut
  error?: string
  //: The backend error's `detail` payload, when the failure came from a
  //: rejected `fetchJson` call - unused by M4a-c callers (additive only;
  //: see `FetchJsonError`), read by the M4f runner to distinguish a genuine
  //: cap denial from every other prepare/confirm failure.
  detail?: unknown
}
...
async function navigatePrepareForTab(
  ...
  try {
    ...
    return {
      ...
      detail: (err as FetchJsonError)?.detail,
    }
  }
}
...
function validBatchTaskIds(value: unknown): value is number[] {
  return Array.isArray(value) && value.length >= 1 && value.length <= BATCH_MAX_TASKS
    && value.every(id => Number.isSafeInteger(id) && id > 0)
    && new Set(value).size === value.length
}

function validBatchPointer(value: unknown): value is BatchPointer {
  if (!value || typeof value !== 'object') return false
  const pointer = value as BatchPointer
...
async function getBatchPointer(): Promise<BatchPointer | null> {
...
async function setBatchPointer(pointer: BatchPointer | null): Promise<void> {
  if (pointer) await chrome.storage.session.set({ [BATCH_STORAGE_KEY]: pointer })
  else await chrome.storage.session.remove(BATCH_STORAGE_KEY)
}

async function patchBatchForTask(taskId: number, patch: Partial<Pick<BatchPointer,
  'state' | 'tabId' | 'lastError'>>): Promise<BatchPointer | null> {
  const current = await getBatchPointer()
  if (!current || current.taskIds[current.currentIndex] !== taskId) return null
  const updated: BatchPointer = { ...current, ...patch, updatedAt: new Date().toISOString() }
  await setBatchPointer(updated)
  return updated
}

// User-authorized local salary fallback; timeouts bound a single foreground request.
...
interface SalaryFrame {
  canonicalUrl: string; title: string; raw: string; nodeId: number; pageUrl: string
  x: number; y: number; width: number; height: number
  viewportWidth: number; viewportHeight: number; scrollX: number; scrollY: number
}
interface SalaryFrameResult { status: string; frame: SalaryFrame | null }
let salaryOcrBusy = false
let lastSalaryScreenshot = 0

const SALARY_BACKFILL_KEY = 'jobagent_salary_backfill_pointer'
interface SalaryBackfillItem { job_id: number; title: string; company: string; source_url: string; state: string }
interface SalaryBackfillRun { id: number; state: string; current_job_id: number | null; paused_reason: string | null;
  session_processed: number; session_cap: number; items: SalaryBackfillItem[] }
interface SalaryBackfillPointer { runId: number; tabId: number; active: boolean; updatedAt: string }
let salaryBackfillBusy = false

async function getSalaryBackfillPointer(): Promise<SalaryBackfillPointer | null> {
  const stored = await chrome.storage.session.get(SALARY_BACKFILL_KEY)
  const value = stored[SALARY_BACKFILL_KEY] as SalaryBackfillPointer | undefined
  return value && Number.isSafeInteger(value.runId) && Number.isSafeInteger(value.tabId) ? value : null
}

async function setSalaryBackfillPointer(value: SalaryBackfillPointer | null): Promise<void> {
  if (value) await chrome.storage.session.set({ [SALARY_BACKFILL_KEY]: value })
  else await chrome.storage.session.remove(SALARY_BACKFILL_KEY)
}

function salaryBounded<T>(operation: Promise<T>, timeout = 8000): Promise<T> {
...
async function salaryForeground(tabId: number): Promise<number> {
  const tab = await salaryBounded(chrome.tabs.get(tabId))
  if (!tab.active || tab.windowId === undefined || !tab.url?.startsWith(REQUIRED_ORIGIN + '/')) throw new Error('salary_tab_changed')
  if (!(await salaryBounded(chrome.windows.get(tab.windowId))).focused) throw new Error('salary_not_foreground')
  return tab.windowId
}
...
async function salaryCrop(screenshot: string, f: SalaryFrame): Promise<string> {
  ...
  if (!values.every(Number.isFinite) || f.x < 0 || f.y < 0 || f.width < 6 || f.width > 400 || f.height < 4 || f.height > 80 ||
      f.viewportWidth <= 0 || f.viewportHeight <= 0 || f.x + f.width > f.viewportWidth || f.y + f.height > f.viewportHeight) throw new Error('salary_bad_region')
  if (!screenshot.startsWith('data:image/png;base64,') || screenshot.length > 32000000) throw new Error('salary_bad_image')
  ...
  try {
    ...
    if (Math.abs(sx - sy) > 0.03 || sx < 0.5 || sx > 4) throw new Error('salary_viewport_changed')
    const canvas = new OffscreenCanvas(Math.ceil(f.width * 2), Math.ceil(f.height * 2))
    const ctx = canvas.getContext('2d')
    if (!ctx) throw new Error('salary_canvas_unavailable')
    ctx.drawImage(bitmap, f.x * sx, f.y * sy, f.width * sx, f.height * sy, 0, 0, canvas.width, canvas.height)
    const blob = await canvas.convertToBlob({ type: 'image/png' })
    if (blob.size > 262144) throw new Error('salary_crop_too_large')
    const cropped = new Uint8Array(await blob.arrayBuffer())
    let binary = ''
    for (const value of cropped) binary += String.fromCharCode(value)
    return btoa(binary)
  } finally { bitmap.close() }
}

/** OCR may fill only a missing salary. It never changes candidate identity or imports. */
async function supplementSalary<T extends SalaryCandidate>(tabId: number, candidate: T, allowed: () => Promise<boolean>): Promise<T> {
  if (candidate.salary_text || !candidate.title || !canonicalizeJobDetailUrl(candidate.source_url) || salaryOcrBusy) return candidate
  if (!chrome.tabs.captureVisibleTab || !chrome.windows?.get) return candidate
  salaryOcrBusy = true
  let contextChanged = false
  const changed = () => { contextChanged = true }
  const updated = (id: number, change: { url?: string; status?: string }) => {
    if (id === tabId && (change.url || change.status === 'loading')) changed()
  }
  chrome.tabs.onActivated?.addListener(changed)
  chrome.tabs.onUpdated?.addListener(updated)
  chrome.windows.onFocusChanged?.addListener(changed)
  const note = (reason: string): T => ({ ...candidate, warnings: [...(candidate.warnings || []), '本地薪资 OCR 未采用：' + reason] })
  try {
    const message = { type: 'jobagent:salary-frame', canonicalUrl: candidate.source_url, expectedTitle: candidate.title }
    const read = async (): Promise<SalaryFrameResult> => {
      if (!await allowed()) throw new Error('salary_cancelled')
      ...
      if (contextChanged) throw new Error('salary_tab_changed')
      await salaryForeground(tabId)
      const response = await salaryBounded(chrome.tabs.sendMessage(tabId, message)) as { ok?: boolean; result?: SalaryFrameResult }
      if (!response.ok || !response.result) throw new Error('salary_frame_unavailable')
      if (response.result.status === 'login_required') throw new Error('salary_login_required')
      if (response.result.status === 'verification') throw new Error('salary_verification')
      return response.result
    }
    const first = await read()
    if (first.status !== 'ok' || !first.frame) return note(first.status)
    const frame = first.frame
    const stable = async () => {
      const next = await read()
      if (next.status !== 'ok' || JSON.stringify(next.frame) !== JSON.stringify(frame)) throw new Error('salary_identity_or_region_changed')
    }
    // Never retry or raise capture frequency when the last attempt was recent.
    if (Date.now() - lastSalaryScreenshot < 600) return note('rate_limited')
    ...
    let result: { salary_text?: string | null; reason?: string }
    try {
    ...
    if (!/^\d{1,3}(?:\.\d{1,2})?-\d{1,3}(?:\.\d{1,2})?[K万](?:·\d{2}薪)?$/.test(salary) ||
        salary.includes('薪') !== frame.raw.includes('薪') || salary.includes('万') !== frame.raw.includes('万')) return note('uncertain')
    ...
    if (['salary_login_required', 'salary_verification', 'salary_cancelled', 'salary_identity_or_region_changed', 'salary_tab_changed', 'salary_not_foreground'].includes(reason)) throw error
...
function canonicalizeJobDetailUrl(raw: string | null | undefined): string | null {
  if (!raw) return null
  let parsed: URL
  try {
    parsed = new URL(raw)
  ...
  if (parsed.origin !== RUNNER_REQUIRED_ORIGIN) return null
  if (!JOB_DETAIL_PATH.test(parsed.pathname)) return null
...
interface RunnerPendingTerminal {
  outcome: 'completed' | 'failed' | 'cancelled'
  error?: string
  //: Whether the backend `run/{complete,fail,cancel}` transition itself is
  //: already confirmed - only the session stop remains. A retry must never
  //: replay an already-confirmed transition (it is not idempotent on the
  //: backend - a second `complete`/`fail` 422s) - CLAUDE.md M4f review item
  //: 2. `true` both when this attempt's own transition succeeded and when
  //: the backend had already moved the task to a terminal state on its own
  //: (`record_round`'s no-new-round auto-completion - review item 1).
  transitionDone: boolean
}

interface RunnerPointer {
  autoMatch?: boolean
  pendingMatch?: { jobId: number; canonicalUrl: string } | null
  taskId: number
  tabId: number
  //: `null` until the `SupervisedSession` backing this run has actually been
  //: created - see `runRunnerLoop`'s ordering note below (CLAUDE.md M4f
  //: review item 6).
  sessionId: number | null
  //: Monotonic per-run-attempt id - see the section banner above.
  runToken: number
  phase: RunnerPhase
  pauseRequested: boolean
  //: Set once the backend has *confirmed* the `pause` transition for the
  //: current `pauseRequested`. `pauseRequested` alone already stops the
  //: loop locally (durable-state re-read at every checkpoint); this tracks
  //: only whether the backend agrees, so a failed confirmation stays
  //: visible (`lastError`) and retryable (`jobagent:runner-retry-pause`) -
  //: CLAUDE.md M4f review item 4.
  ...
  handledUrls: string[]
  // Run-local discovery history, not a second Job store. Query-free identities only;
  // retained across explicit resume so virtualized/reappearing cards are not "new" again.
  seenCandidateUrls?: string[]
  //: Cumulative task-level counters mirrored from the backend's own
  //: `SearchPlanTaskOut` (`observed_count`/`new_count`/`duplicate_count`/
  //: `no_new_rounds`) - refreshed on start and after every recorded round,
  //: so the overlay/popup can show them without polling - CLAUDE.md M4f
  //: review item 6.
  observedCount: number
  newCount: number
  duplicateCount: number
  noNewRounds: number
  //: Mirrored to the backend via `POST /run/state` (`reportState`) at every
  //: real checkpoint - never a query string (BOSS session tokens live
  //: there; `reportState`'s canonicalization strips it before sending), the
  //: cards currently rendered on the page, how many this run has actually
  //: imported through `job_intake`, the title of whatever candidate is
  //: presently being opened/read, and a short label for the most recent
  //: step - CLAUDE.md M4f observability delta.
  currentUrl: string | null
  visibleJobs: number
  importedJobs: number
  currentCandidate: string | null
  lastAction: string | null
  //: Why the run is currently paused/verification-halted, if it is - never
  //: an actual failure (that is `lastError`).
  pausedReason: string | null
  //: The task's own search criteria - set once from `RunnerTaskOut` at
  //: start and never re-derived, so the popup/overlay can show which
  //: city/keyword this run is actually for without a second fetch.
  city: string | null
  keyword: string | null
  ...
  updatedAt: string
  //: Surfaces a recoverable failure (CLAUDE.md M4f review item 3) - never
  //: silently swallowed, never used to guess at automatic recovery.
  lastError: string | null
  //: Set only when a terminal backend transition or session-stop could not
  //: be confirmed - the pointer is deliberately *not* cleared in that case
  //: (see `endRunner`) so the human can retry via
  //: `jobagent:runner-retry-terminal` instead of losing the run's state.
  pendingTerminal: RunnerPendingTerminal | null
}
...
async function getRunnerPointer(): Promise<RunnerPointer | null> {
  const stored = await chrome.storage.session.get(RUNNER_STORAGE_KEY)
  return (stored[RUNNER_STORAGE_KEY] as RunnerPointer | undefined) || null
}

let runnerWriteQueue: Promise<void> = Promise.resolve()
function setRunnerPointer(pointer: RunnerPointer | null): Promise<void> {
  ...
  runnerWriteQueue = write.catch(() => { /* allow a later explicit recovery write */ })
  return write
}
...
function isRunnerNavOrigin(url: string): boolean {
  try {
...
async function reusableBossTab(windowId: number): Promise<chrome.tabs.Tab | undefined> {
  const tabs = await chrome.tabs.query({ windowId })
  return tabs.find((tab) => {
    if (tab.id === undefined || !tab.url || !isRunnerNavOrigin(tab.url)) return false
    try {
...
function claimRunnerLoop(runToken: number): boolean {
  if (activeRunToken !== null) return false
  activeRunToken = runToken
  return true
}

function releaseRunnerLoop(runToken: number): void {
...
async function askTab<T>(
  ...
  try {
    ...
    try {
      ...
      return (retry || { ok: false }) as { ok: boolean; result?: T; error?: string }
    } catch { return { ok: false, error: 'content_unavailable' } }
  }
}

async function fetchRunnerTask(taskId: number): Promise<RunnerTaskOut> {
  return fetchJson<RunnerTaskOut>(`/api/tasks/search-plan/${taskId}`)
}

async function postTaskRun(taskId: number, action: string, body?: unknown): Promise<unknown> {
...
function queryFreeUrl(url: string): string | null {
  try {
 ...
 */
function reportState(taskId: number, report: RunnerStateReport): void {
  const body: RunnerStateReport = { ...report }
...
//: page.
function sameSearchTarget(currentUrl: string, targetUrl: string): boolean {
  let current: URL
  let target: URL
  try {
...
function toStateMessage(pointer: RunnerPointer): RunnerStateMessage {
  return {
...
function broadcastRunnerState(pointer: RunnerPointer): void {
  chrome.tabs
...
function broadcastRunnerCleared(tabId: number): void {
...
async function readCurrent(taskId: number, runToken: number): Promise<RunnerPointer | null> {
  const pointer = await getRunnerPointer()
  if (!pointer || pointer.taskId !== taskId || pointer.runToken !== runToken) return null
  return pointer
}
...
async function persistPatch(
  ...
  const merged: RunnerPointer = { ...fresh, ...patch, updatedAt: new Date().toISOString() }
...
async function persistAndReport(
  ...
  const report: RunnerStateReport = {}
  if (patch.currentUrl !== undefined) report.current_url = patch.currentUrl
  if (patch.scrollsUsed !== undefined) report.scroll_round = patch.scrollsUsed
  if (patch.visibleJobs !== undefined) report.visible_jobs = patch.visibleJobs
  if (patch.importedJobs !== undefined) report.imported_jobs = patch.importedJobs
  if (patch.currentCandidate !== undefined) report.current_candidate = patch.currentCandidate
  if (patch.lastAction) report.last_action = patch.lastAction
  if (Object.keys(report).length) reportState(taskId, report)
  return updated
}

function detectedCandidateUrls(result: RunnerDetectResult): Set<string> {
...
async function rememberRenderedCandidates(
  taskId: number, runToken: number, result: RunnerDetectResult,
): Promise<RunnerPointer | null> {
  const fresh = await readCurrent(taskId, runToken)
  if (!fresh) return null
...
type CheckpointResult = 'continue' | 'pause' | 'cancel' | 'superseded'

/** The one place every bounded step checks for a stop request - always a
 * fresh read of durable state, never a closed-over flag. */
async function checkpoint(taskId: number, runToken: number): Promise<CheckpointResult> {
  const fresh = await readCurrent(taskId, runToken)
  if (!fresh) return 'superseded'
  if (fresh.cancelRequested) return 'cancel'
  if (fresh.pauseRequested) return 'pause'
  return 'continue'
}
...
async function endRunner(
  ...
  if (!transitionAlreadyDone) {
    try {
      if (outcome === 'completed') await postTaskRun(taskId, 'complete')
      else if (outcome === 'failed') await postTaskRun(taskId, 'fail', { error: error || 'unknown' })
      else await postTaskRun(taskId, 'cancel')
    } catch {
      transitionOk = false
    }
  }
...
async function finishBatchTask(
  finished: RunnerPointer,
  outcome: 'completed' | 'failed' | 'cancelled',
  error?: string,
): Promise<void> {
  const batch = await getBatchPointer()
  if (!batch || batch.taskIds[batch.currentIndex] !== finished.taskId
    || batch.tabId !== finished.tabId || !['running', 'paused'].includes(batch.state)) return
 ...
 */
async function pauseForHumanGate(
  taskId: number,
  runToken: number,
  reason: 'verification' | 'login_required',
): Promise<void> {
  ...
  try {
    ...
    await persistPatch(taskId, runToken, {
      lastError: `${reason}_transition_failed:` + String((err as Error)?.message || err),
    })
  }
  releaseRunnerLoop(runToken)
}
 ...
 */
async function haltForStop(taskId: number, runToken: number): Promise<void> {
  const fresh = await readCurrent(taskId, runToken)
  if (!fresh) {
    releaseRunnerLoop(runToken)
    return
...
type StabilizeOutcome =
  | { ok: true; result: RunnerDetectResult }
  | { ok: false; reason: 'stopped' }
  | { ok: false; reason: 'verification' }
  | { ok: false; reason: 'login_required' }
  | { ok: false; reason: 'failed'; error: TabCheckFailure | `stabilization_timeout:${string}` }
...
async function waitForStableSearchPage(
  taskId: number,
  runToken: number,
  tabId: number,
  targetUrl: string,
): Promise<StabilizeOutcome> {
  let pendingReason: 'target_mismatch' | 'content_unavailable' | 'unsupported_page' = 'target_mismatch'
  for (let attempt = 0; attempt < RUNNER_STABILIZE_MAX_ATTEMPTS; attempt++) {
    const cp = await checkpoint(taskId, runToken)
    if (cp === 'superseded') return { ok: false, reason: 'stopped' }
    if (cp === 'cancel' || cp === 'pause') return { ok: false, reason: 'stopped' }
    ...
    const detected = await askTab<RunnerDetectResult>(tabId, { type: 'jobagent:detect' }, attempt === 0)
    if (await checkpoint(taskId, runToken) !== 'continue') return { ok: false, reason: 'stopped' }
    ...
    if (detected.ok && detected.result) {
      if (detected.result.login_required) return { ok: false, reason: 'login_required' }
      if (detected.result.verification) return { ok: false, reason: 'verification' }
      if (detected.result.page_type === 'search') return { ok: true, result: detected.result }
 ...
 */
async function processNewCandidates(
  taskId: number,
  runToken: number,
  tabId: number,
): Promise<CandidateRoundOutcome> {
  for (;;) {
    const cp = await checkpoint(taskId, runToken)
    ...
    if (!validRunnerBudget(fresh)) return { status: 'error', reason: 'invalid_candidate_budget' }
    ...
    if (fresh.autoMatch && fresh.pendingMatch) {
      ...
      try {
        ...
        const outcome = await salaryBounded(fetchJson<{ state: string }>(`/api/tasks/${taskId}/auto-match/step`, {
          method: 'POST', body: { job_id: fresh.pendingMatch.jobId, canonical_url: fresh.pendingMatch.canonicalUrl },
        }), 180000)
        if (outcome.state === 'running') return { status: 'error', reason: 'match_result_uncertain_no_retry' }
        if (outcome.state !== 'done' && outcome.state !== 'failed') return { status: 'error', reason: 'match_invalid_response' }
    ...
    if (detected.result.page_type !== 'search') return { status: 'wrong_page' }
    ...
    if (!prepared.ok) {
      ...
      if (isCapDenied(prepared, 'candidate_cap')) return { status: 'cap_reached' }
    ...
    let reserved: RunnerPointer | null
    try {
    ...
    if (!afterHandled) return { status: 'stopped' } // superseded/cleared mid-step
    ...
    const confirmed = await navigateConfirmForTab(
      ...
      clicked ? null : opened.result?.error || 'unknown',
    )
    if (!clicked) return { status: 'error', reason: opened.error || 'click_failed' }
    ...
    for (let attempt = 0; attempt < RUNNER_CAPTURE_MAX_ATTEMPTS; attempt++) {
      ...
      if (!cap.ok) return { status: 'error', reason: cap.error || 'capture_failed' } // tab lost/wrong origin/backgrounded - fail closed
    ...
    if (!merged.salary_text) {
      try {
        ...
        if (freshCapture.result?.status === 'verification') return { status: 'verification' }
        if (!freshCapture.ok || freshCapture.result?.status !== 'ok') return { status: 'error', reason: 'salary_identity_recheck_failed' }
      } catch (error) {
        const reason = error instanceof Error ? error.message : 'salary_failed'
        if (reason === 'salary_login_required') return { status: 'login_required' }
        if (reason === 'salary_verification') return { status: 'verification' }
        if (reason === 'salary_cancelled') return { status: 'stopped' }
    ...
    try {
      const preview = (await fetchJson('/api/extension/jobs/preview', {
        ...
        rows?: { status?: string; existing_job_id?: number | null; enrichable_fields?: string[] }[]
      }
      ...
      if (!intakeIncomplete && !intakeExcluded && ((preview.new_count ?? 0) > 0 || needsSalaryEnrichment)) {
        ...
        })) as { job_id?: number; duplicate?: boolean }
        didImport = imported.duplicate !== true // enrichment is not a new import
        resolvedJobId = imported.job_id ?? null
      }
    } catch (err) {
      return { status: 'error', reason: 'intake_failed:' + String((err as Error)?.message || err) }
    }
    ...
    if (resolvedJobId === null) return { status: 'error', reason: 'missing_job_identity' }
    try {
      ...
      return {
        status: 'error',
        reason: 'candidate_association_failed:' + String((err as Error)?.message || err),
      }
    }

    if (fresh.autoMatch) await persistPatch(taskId, runToken, {
    ...
    if (!afterProgress) return { status: 'stopped' }
  }
}
...
async function runScrollRound(
  ...
  | { status: 'ok'; observed: number; new: number; duplicate: number }
  | { status: 'verification' }
  | { status: 'login_required' }
  | { status: 'wrong_page' }
  | { status: 'stopped' }
...
> {
  ...
  if (!inventory) return { status: 'stopped' }
  ...
  if (!prepared.ok) {
    ...
    // real error.
    if (isCapDenied(prepared, 'scroll_cap')) return { status: 'cap_reached' }
    return { status: 'error', reason: prepared.error || 'prepare_denied' }
  }
  ...
  if (!budget) return { status: 'stopped' }
  if (budget.scrollsUsed >= RUNNER_MAX_SCROLL_ROUNDS) return { status: 'cap_reached' }
  try {
    const reserved = await persistAndReport(taskId, runToken, { scrollsUsed: budget.scrollsUsed + 1 })
    if (!reserved || await checkpoint(taskId, runToken) !== 'continue') return { status: 'stopped' }
  } catch {
  ...
  const confirmed = await navigateConfirmForTab(
    ...
    succeeded ? null : scrolled.result?.error || 'unknown',
  )
  if (!succeeded) return { status: 'error', reason: scrolled.error || 'scroll_failed' }
  ...
  let stableResult: RunnerDetectResult | null = null
  for (let attempt = 0; attempt < RUNNER_STABILIZE_MAX_ATTEMPTS; attempt++) {
    const cp = await checkpoint(taskId, runToken)
    if (cp !== 'continue') return { status: 'stopped' }
  ...
  if (!stableResult) return { status: 'error', reason: 'stabilize_timeout' }

  const afterUrls = detectedCandidateUrls(stableResult)
  let newCount = 0
  for (const url of afterUrls) if (!seenUrls.has(url)) newCount += 1
  const duplicateCount = afterUrls.size - newCount
  if (!await rememberRenderedCandidates(taskId, runToken, stableResult)) return { status: 'stopped' }

  return { status: 'ok', observed: afterUrls.size, new: newCount, duplicate: duplicateCount }
}

/** Resolve the foreground start target without relying on implicit worker scope. */
type ForegroundStartResult = { ok: true; tab: chrome.tabs.Tab } | { ok: false; error: string }
interface PopupBinding { tabId: number; windowId: number }

/** Only a real action popup may defer focus validation until it closes. */
async function bindPopupTarget(value: unknown, sender: chrome.runtime.MessageSender): Promise<PopupBinding> {
  const target = value as { tabId?: number; windowId?: number } | undefined
  ...
  if (contexts.length !== 1 || contexts[0].documentUrl !== sender.url) throw new Error('start-v4/popup_not_open')
  return { tabId: target.tabId!, windowId: target.windowId! }
}

async function foregroundStartTab(binding?: PopupBinding): Promise<ForegroundStartResult> {
  // Fixed, non-private reason codes distinguish live failures and loaded builds.
  const refuse = (reason: string): ForegroundStartResult => ({ ok: false, error: 'start-v3/' + reason })
  ...
  try {
    ...
    if (window.id === undefined) return refuse('window_id_missing')
    if (window.type !== 'normal') return refuse('window_not_normal')
    if (!binding && !window.focused) return refuse('window_not_focused')
    if (binding && binding.windowId !== window.id) return refuse('tab_window_mismatch')
    stage = 'tab_query'
    const tabs = await chrome.tabs.query({ active: true, windowId: window.id })
    if (!tabs.length) return refuse('active_tab_missing')
    if (tabs.length !== 1) return refuse('active_tab_ambiguous')
    if (!tabs[0].active) return refuse('tab_not_active')
    if (tabs[0].windowId !== window.id) return refuse('tab_window_mismatch')
    if (binding && tabs[0].id !== binding.tabId) return refuse('tab_not_active')
    stage = 'focus_recheck'
    if (!binding && !(await chrome.windows.get(window.id)).focused) return refuse('window_focus_changed')
    return { ok: true, tab: tabs[0] }
  } catch { return refuse(stage + '_failed') }
}
...
async function waitForPopupHandoff(binding: PopupBinding, initialUrl: string, stopped: () => Promise<boolean>): Promise<string | null> {
  try {
    for (let attempt = 0; attempt < 20; attempt++) {
      ...
      if (state.popups.length === 0 && state.window.focused) return null
    }
    return 'start-v4/handoff_focus_timeout'
  } catch { return 'start-v4/handoff_query_failed' }
}

async function afterPopupHandoff(pointer: RunnerPointer, binding: PopupBinding, initialUrl: string, proceed: () => Promise<void>): Promise<void> {
  ...
  const cp = await checkpoint(taskId, runToken)
  if (cp === 'superseded') { releaseRunnerLoop(runToken); return }
  if (cp === 'cancel' || cp === 'pause') { await haltForStop(taskId, runToken); return }
  if (reason) { await endRunner(taskId, runToken, 'failed', reason); return }
  await proceed()
}
...
async function startRunner(taskId: number, candidateCap: unknown, binding?: PopupBinding,
  ...
  if (await getRunnerPointer()) return fail('already_running')
  if (await getGlobalPointer()) return fail('session_already_active')
  const batch = await getBatchPointer()
  if (batch && ['running', 'paused'].includes(batch.state)) {
    if (!batchEntry || batch.taskIds[batch.currentIndex] !== taskId) return fail('batch_already_active')
  } else if (batchEntry) return fail('batch_not_active')
  ...
  let consoleTab: chrome.tabs.Tab | undefined
  if (consoleSender) {
    try {
      const task = await fetchRunnerTask(taskId)
      if (task.run_status !== 'pending') return fail('任务不是待开始状态；已结束的任务不会自动重跑。')
      if (!task.search_url || !isConsoleSearchUrl(task.search_url)) return fail('wrong_search_url')
      if (task.max_candidates != null && (!validCandidateCap(task.max_candidates)
        || candidateCap > task.max_candidates)) return fail('invalid_task_candidate_cap')
      ...
      if (consoleTab.id === undefined || consoleTab.windowId !== source.windowId) return fail('console_tab_create_failed')
      ...
      for (let attempt = 0; attempt < 30; attempt++) {
        ...
        if (!target.active || target.windowId !== source.windowId
          || !(await chrome.windows.get(source.windowId!)).focused) return fail('console_tab_changed')
        if (target.url === task.search_url) { ready = true; break }
      ...
      if (!ready) return fail('console_navigation_timeout')
  ...
  if (task.max_candidates != null) {
    if (!Number.isInteger(task.max_candidates) || task.max_candidates < 1) return fail('invalid_task_candidate_cap')
    if (candidateCap > task.max_candidates) return fail('候选上限超过任务允许的数量，请降低后重试。')
  }
  ...
  if (batchEntry) await patchBatchForTask(taskId, { tabId, state: 'running', lastError: null })
  broadcastRunnerState(pointer)

  if (launch) {
    if (binding) void afterPopupHandoff(pointer, binding, tab.url, () => runRunnerLoop(taskId, runToken, tabId, task.search_url!))
    else void runRunnerLoop(taskId, runToken, tabId, task.search_url, !!consoleTab)
  }
  return { ok: true }
}
...
async function runRunnerLoop(
  ...
  try {
    if (!alreadyNavigated) await chrome.tabs.update(tabId, { url: searchUrl })
  } catch (err) {
    await endRunner(taskId, runToken, 'failed', 'navigate_failed:' + String((err as Error)?.message || err))
    return
  }
  ...
  if (!stabilized.ok) {
    if (stabilized.reason === 'stopped') await haltForStop(taskId, runToken)
    else if (stabilized.reason === 'verification') await pauseForHumanGate(taskId, runToken, 'verification')
    else if (stabilized.reason === 'login_required') await pauseForHumanGate(taskId, runToken, 'login_required')
    else await endRunner(taskId, runToken, 'failed', stabilized.error)
    return
  }
  ...
  if (!approved) { releaseRunnerLoop(runToken); return }
  if (!validRunnerBudget(approved)) {
    await endRunner(taskId, runToken, 'failed', 'invalid_candidate_budget')
    return
  }
  try {
    session = await fetchJson<SessionOut>('/api/extension/sessions', {
      method: 'POST',
      body: {
        task_id: taskId,
        page_cap: 1,
    ...
    await endRunner(taskId, runToken, 'failed', 'session_create_failed:' + String((err as Error)?.message || err))
    return
  }
 ...
 */
async function runRoundsLoop(taskId: number, runToken: number, tabId: number): Promise<void> {
  ...
  for (let round = startRound; round <= RUNNER_MAX_SCROLL_ROUNDS; round++) {
    ...
    try {
      ...
      })) as typeof roundRecorded
    } catch (err) {
      await endRunner(taskId, runToken, 'failed', 'round_record_failed:' + String((err as Error)?.message || err))
      return
    }
...
async function requestRunnerPause(): Promise<{ ok: boolean }> {
  ...
  try {
    ...
    await persistPatch(merged.taskId, merged.runToken, {
      lastError: 'pause_transition_failed:' + String((err as Error)?.message || err),
    })
    return { ok: true }
  }
  ...
  if (confirmed) {
    const withConfirm: RunnerPointer = { ...confirmed, pauseConfirmed: true }
...
async function requestRunnerCancel(): Promise<{ ok: boolean }> {
  const fresh = await getRunnerPointer()
  if (!fresh) return { ok: false }
  const merged: RunnerPointer = { ...fresh, cancelRequested: true, updatedAt: new Date().toISOString() }
  await setRunnerPointer(merged)
  broadcastRunnerState(merged)
 ...
 * check regardless. */
async function resumeRunner(binding?: PopupBinding, consoleTaskId?: number): Promise<{ ok: boolean; error?: string }> {
  if (salaryOcrBusy) return { ok: false, error: 'salary_ocr_busy' }
  if (activeRunToken !== null) return { ok: false, error: 'already_running' }
  ...
  const pointer = await getRunnerPointer()
  if (!pointer) return fail('not_running')
  ...
  if (!pointer.pauseRequested && !pointer.cancelRequested) return fail('not_paused')
  if (pointer.cancelRequested) return fail('cancelled')
  let resumeUrl = ''
  let pendingSearchUrl: string | null = null
  if (binding) {
    if (binding.tabId !== pointer.tabId) return fail('start-v4/handoff_tab_changed')
    const resolved = await foregroundStartTab(binding)
    if (!resolved.ok) return fail(resolved.error)
    if (!resolved.tab.url || !isRunnerNavOrigin(resolved.tab.url)) return fail('wrong_origin')
  ...
  if (pointer.sessionId === null) {
    try {
      ...
      if (!task.search_url || !isRunnerNavOrigin(task.search_url)) return fail('wrong_origin')
      pendingSearchUrl = task.search_url
    } catch { return fail('resume_task_unavailable') }
  }
  ...
  try {
    ...
    await persistPatch(pointer.taskId, pointer.runToken, {
      lastError: 'resume_transition_failed:' + String((err as Error)?.message || err),
    })
    return fail(String((err as Error)?.message || err))
  }

  const updated: RunnerPointer = {
    ...pointer,
    runToken: newToken,
    pauseRequested: false,
    pauseConfirmed: false,
  ...
  if (binding) void afterPopupHandoff(updated, binding, resumeUrl, proceed)
  else void proceed()
  return { ok: true }
}
...
async function retryRunnerTerminal(): Promise<{ ok: boolean; error?: string }> {
  const pointer = await getRunnerPointer()
  if (!pointer || !pointer.pendingTerminal) return { ok: false, error: 'nothing_pending' }
  ...
  if (after && after.pendingTerminal) return { ok: false, error: after.lastError || 'still_pending' }
  return { ok: true }
}
...
async function consoleSourceTab(sender: chrome.runtime.MessageSender, foreground: boolean): Promise<chrome.tabs.Tab> {
  if (sender.frameId !== 0 || !isConsoleUrl(sender.url) || sender.tab?.id === undefined) throw new Error('untrusted_console')
  const tab = await chrome.tabs.get(sender.tab.id)
  if (!isConsoleUrl(tab.url) || tab.windowId === undefined) throw new Error('console_changed')
  if (foreground) {
    const window = await chrome.windows.getLastFocused({ windowTypes: ['normal'] })
    if (!tab.active || !window.focused || window.type !== 'normal' || window.id !== tab.windowId) throw new Error('console_not_foreground')
  }
  return tab
}
...
async function startBatch(
  ...
  try {
    ...
    if (existing && ['running', 'paused'].includes(existing.state)) return { ok: false, error: '已有批次运行。' }
    // Validate the whole approved list before the first browser side effect.
    const validatedTasks: RunnerTaskOut[] = []
    for (const taskId of taskIds) {
      let task: RunnerTaskOut
      try { task = await fetchRunnerTask(taskId) } catch { return { ok: false, error: '批次任务读取失败；尚未启动。' } }
      ...
      if (task.max_candidates != null && (!validCandidateCap(task.max_candidates)
        || candidateCap > task.max_candidates)) return { ok: false, error: `任务 #${taskId} 的候选上限不足；尚未启动。` }
...
async function resumeBatch(
  ...
  if (pointer.autoMatch) return { ok: false, error: '批次入口不恢复付费任务。' }
  if (activeRunToken !== null) return { ok: false, error: '批次仍在运行，无需恢复。' }
  let target: chrome.tabs.Tab
  try { target = await chrome.tabs.get(pointer.tabId) } catch { return { ok: false, error: '原 BOSS 标签页已关闭，批次不能恢复。' } }
  ...
  if (!foreground.ok || foreground.tab.id !== pointer.tabId) return { ok: false, error: 'BOSS 标签页未处于前台。' }
  ...
  if (task.run_status !== 'running') return { ok: false, error: '当前批次任务已不在运行状态，不能恢复。' }
  const newToken = nextRunToken()
  if (!claimRunnerLoop(newToken)) return { ok: false, error: 'already_running' }
  let pendingSearchUrl: string | null = null
  if (pointer.sessionId === null) {
  ...
  await patchBatchForTask(updated.taskId, { state: 'running', lastError: null })
  broadcastRunnerState(updated)
  if (pendingSearchUrl) void runRunnerLoop(updated.taskId, newToken, updated.tabId, pendingSearchUrl)
  else void runRoundsLoop(updated.taskId, newToken, updated.tabId)
  return { ok: true }
}

async function runSalaryBackfillLoop(runId: number, tabId: number): Promise<void> {
  if (salaryBackfillBusy) return
  salaryBackfillBusy = true
  try {
    while (true) {
      const pointer = await getSalaryBackfillPointer()
      if (!pointer || pointer.runId !== runId || pointer.tabId !== tabId || !pointer.active) return
      let run = await fetchJson<SalaryBackfillRun>(`/api/jobs/salary-backfill/runs/${runId}/claim`, { method: 'POST' })
      ...
      for (let attempt = 0; attempt < RUNNER_CAPTURE_MAX_ATTEMPTS; attempt += 1) {
        const response = await askTab<RunnerDetectResult>(tabId, { type: 'jobagent:detect' }, attempt === 0)
      ...
      let outcome: 'updated' | 'unavailable' | 'failed' = 'unavailable'
      ...
      if (detected?.candidates[0]) {
        ...
        try {
          ...
          if (candidate.salary_text) {
            const preview = await fetchJson<{ rows?: { status?: string; existing_job_id?: number | null;
              enrichable_fields?: string[] }[] }>('/api/extension/jobs/preview', {
            ...
            if (imported.job_id !== item.job_id || imported.duplicate !== true) throw new Error('intake_result_mismatch')
            outcome = 'updated'
            reason = 'salary_enriched'
          } else reason = 'salary_unreadable'
        } catch (error) {
      ...
      run = await fetchJson<SalaryBackfillRun>(`/api/jobs/salary-backfill/runs/${runId}/item`, {
    ...
    try {
      const run = await fetchJson<SalaryBackfillRun>(`/api/jobs/salary-backfill/runs/${runId}`)
      if (run.state === 'running') await fetchJson(`/api/jobs/salary-backfill/runs/${runId}/pause`, {
        method: 'POST', body: { reason: 'worker_error' },
      })
    } catch { /* backend state remains observable; never guess success */ }
    const pointer = await getSalaryBackfillPointer()
    if (pointer?.runId === runId) await setSalaryBackfillPointer({ ...pointer, active: false,
      updatedAt: new Date().toISOString() })
...
async function startSalaryBackfill(runId: number, source: chrome.tabs.Tab): Promise<{ ok: boolean; error?: string }> {
  ...
  if (existing && existing.runId !== runId) return { ok: false, error: '已有其他薪资回填计划。' }
...
interface ApplicationApprovalWire {
  id: number
  job_id: number
  company: string
  title: string
...
type M6PageResult = {
  status: 'ok' | 'clicked' | 'login_required' | 'verification' | 'wrong_page'
    | 'identity_mismatch' | 'control_missing' | 'control_ambiguous'
    | 'control_disabled' | 'control_wrong_state'
  observed_url?: string
  observed_external_id?: string
}

let m6AttemptBusy = false
let m7ScanBusy = false
...
type M7ChatSnapshot = {
  ...
  source_url?: string | null
  external_id?: string | null
  messages?: { source_message_id: string; direction: 'recruiter' | 'user'; text: string;
    source_message_time_text: string | null }[]
}

type M7ScanTotals = { conversations: number; matched: number; skipped: number;
  observed: number; imported: number; duplicates: number; scroll_rounds: number; ai_used: false }

async function pushM7Status(tabId: number, phase: 'scanning' | 'complete' | 'failed',
  totals: M7ScanTotals, detail?: string): Promise<void> {
  try {
    await chrome.tabs.sendMessage(tabId, {
      type: 'jobagent:m7-status', state: { phase, ...totals, detail: detail || null },
    })
  } catch { /* Best-effort display only; it never changes scan authority or outcome. */ }
}

const M7_CHAT_URL = 'https://www.zhipin.com/web/geek/chat'

/** Resolve the one M7 execution surface without disturbing an existing BOSS
 * search/detail tab. A human-confirmed console action may create one new,
 * active, fixed-URL chat tab when none exists. Navigation readiness is bounded
 * and every observation stays tied to that exact tab and focused window. */
async function prepareM7ChatTab(source: chrome.tabs.Tab): Promise<
  { ok: true; tab: chrome.tabs.Tab } | { ok: false; error: string }
> {
  if (source.windowId === undefined) return { ok: false, error: '找不到当前正常 Chrome 窗口。' }
  const windowId = source.windowId
  const candidates = (await chrome.tabs.query({ windowId })).filter(tab => {
    try { return !!tab.id && new URL(tab.url || '').origin === REQUIRED_ORIGIN
      && new URL(tab.url || '').pathname === '/web/geek/chat' } catch { return false }
  })
  if (candidates.length > 1) {
    return { ok: false, error: '同一窗口存在多个 BOSS 聊天页，请只保留一个。' }
  }
  let target = candidates.length === 1
    ? await chrome.tabs.update(candidates[0].id!, { active: true })
    : await chrome.tabs.create({ url: M7_CHAT_URL, active: true, windowId })
  if (target.id === undefined || target.windowId !== windowId) {
    return { ok: false, error: '无法在当前 Chrome 窗口打开 BOSS 消息页。' }
  }
  const tabId = target.id
  for (let attempt = 0; attempt < 40; attempt++) {
    target = await chrome.tabs.get(tabId)
    if (!target.active || target.windowId !== windowId
      || !(await chrome.windows.get(windowId)).focused) {
      return { ok: false, error: 'BOSS 消息页没有保持前台；没有读取。' }
    }
    try {
      const url = new URL(target.url || '')
      if (url.origin === REQUIRED_ORIGIN && url.pathname === '/web/geek/chat') {
        return { ok: true, tab: target }
      }
      if (url.protocol !== 'about:' && url.origin !== REQUIRED_ORIGIN) {
        return { ok: false, error: 'BOSS 消息页跳转到了非预期网站；没有读取。' }
      }
    } catch { /* Chrome may briefly expose an empty URL after tabs.create. */ }
    await sleepMs(250)
  }
  return { ok: false, error: 'BOSS 未能进入消息页，可能需要先登录或完成人工验证。' }
}

async function scanCurrentBossChat(
  source: chrome.tabs.Tab,
): Promise<{ ok: boolean; error?: string; scan?: unknown }> {
  if (m7ScanBusy) return { ok: false, error: '已有当前对话扫描正在进行。' }
  m7ScanBusy = true
  try {
    if (activeRunToken !== null || salaryBackfillBusy || m6AttemptBusy
      || await getRunnerPointer() || await getGlobalPointer()) {
      return { ok: false, error: '已有浏览器任务运行；当前对话扫描不会并发执行。' }
    }
    const prepared = await prepareM7ChatTab(source)
    if (!prepared.ok) return prepared
    const target = prepared.tab
    const alive = await verifyRunnerTab(target.id!)
    if (!alive.ok) return { ok: false, error: 'BOSS 聊天页未保持前台；没有读取。' }
    let resetStatus = 'unavailable'
    for (let attempt = 0; attempt < 20; attempt++) {
      const reset = await askTab<{ status: string }>(target.id!, {
        type: 'jobagent:m7-reset-chat-list',
      }, attempt === 0)
      resetStatus = reset.result?.status || reset.error || 'unavailable'
      if (resetStatus === 'reset') break
      if (resetStatus === 'login_required' || resetStatus === 'verification') {
        return { ok: false, error: `BOSS 要求${resetStatus === 'login_required' ? '登录' : '人工验证'}；已停止。` }
      }
      if (!['list_missing', 'content_unavailable'].includes(resetStatus)) break
      await sleepMs(250)
    }
    const totals: M7ScanTotals = { conversations: 0, matched: 0, skipped: 0, observed: 0,
      imported: 0, duplicates: 0, scroll_rounds: 0, ai_used: false }
    if (resetStatus !== 'reset') {
      const error = `BOSS 消息列表尚未就绪：${resetStatus}。`
      await pushM7Status(target.id!, 'failed', totals, error)
      return { ok: false, error }
    }
    await sleepMs(250)
    await pushM7Status(target.id!, 'scanning', totals, '正在读取已渲染沟通列表。')
    while (totals.conversations < 50) {
      const foreground = await verifyRunnerTab(target.id!)
      if (!foreground.ok) {
        const error = '扫描时 BOSS 聊天页失去前台；已停止。'
        await pushM7Status(target.id!, 'failed', totals, error)
        return { ok: false, error }
      }
      const selected = await askTab<{ status: string; recruiter_name?: string | null }>(target.id!, {
        type: 'jobagent:m7-select-next-chat',
      }, true)
      const selectStatus = selected.result?.status || selected.error || 'unavailable'
      if (selectStatus === 'exhausted') {
        if (totals.scroll_rounds >= 5) break
        const scrolled = await askTab<{ status: string }>(target.id!, {
          type: 'jobagent:m7-scroll-chat-list',
        }, true)
        if (scrolled.result?.status === 'end') break
        if (scrolled.result?.status !== 'scrolled') {
          const error = `沟通列表滚动失败：${scrolled.result?.status || scrolled.error || 'unavailable'}。`
          await pushM7Status(target.id!, 'failed', totals, error)
          return { ok: false, error }
        }
        totals.scroll_rounds += 1
        await pushM7Status(target.id!, 'scanning', totals, '正在加载下一组已渲染联系人。')
        await sleepMs(500)
        continue
      }
      if (selectStatus !== 'selected') {
        const error = `沟通列表读取失败：${selectStatus}。`
        await pushM7Status(target.id!, 'failed', totals, error)
        return { ok: false, error }
      }

      let snapshot: M7ChatSnapshot | undefined
      for (let attempt = 0; attempt < 5; attempt++) {
        await sleepMs(350)
        const captured = await askTab<M7ChatSnapshot>(target.id!, {
          type: 'jobagent:m7-scan-current-chat',
        }, true)
        if (captured.result?.status === 'login_required' || captured.result?.status === 'verification') {
          const error = `BOSS 要求${captured.result.status === 'login_required' ? '登录' : '人工验证'}；已停止。`
          await pushM7Status(target.id!, 'failed', totals, error)
          return { ok: false, error }
        }
        if (captured.ok && captured.result?.status === 'ok' && captured.result.messages
          && captured.result.page_url
          && (!selected.result?.recruiter_name || captured.result.recruiter_name === selected.result.recruiter_name)) {
          snapshot = captured.result
          break
        }
      }
      if (!snapshot) {
        const error = '切换对话后身份未稳定；已停止且不会自动重试。'
        await pushM7Status(target.id!, 'failed', totals, error)
        return { ok: false, error }
      }
      const beforeWrite = await verifyRunnerTab(target.id!)
      if (!beforeWrite.ok) {
        const error = '写入前丢失前台 BOSS 对话；已停止。'
        await pushM7Status(target.id!, 'failed', totals, error)
        return { ok: false, error }
      }
      const scan = await fetchJson<{ conversation_id: number | null; matched: boolean;
        observed: number; imported: number; duplicates: number; ai_used: false }>(
        '/api/recruiter-conversations/boss-current-scan', { method: 'POST', body: snapshot },
      )
      totals.conversations += 1
      totals.observed += scan.observed
      totals.imported += scan.imported
      totals.duplicates += scan.duplicates
      if (scan.matched) totals.matched += 1
      else totals.skipped += 1
      await pushM7Status(target.id!, 'scanning', totals, '正在继续读取沟通列表。')
    }
    await pushM7Status(target.id!, 'complete', totals,
      totals.imported ? '扫描完成，新增消息已写入 HR沟通。' : '扫描完成，没有可新增并唯一关联的消息。')
    return { ok: true, scan: totals }
  } catch {
    return { ok: false, error: '当前对话扫描中断；不会自动重试。' }
  } finally { m7ScanBusy = false }
}
...
async function openM6Detail(tabId: number, canonicalUrl: string): Promise<boolean> {
  ...
  return new Promise<boolean>((resolve) => {
    ...
    const updated = (changedId: number, change: { url?: string; status?: string }) => {
      if (changedId === tabId && change.status === 'complete') finish(true)
    }
    chrome.tabs.onUpdated.addListener(updated)
...
async function executeM6Application(
  ...
  try {
    ...
    await fetchJson<ApplicationApprovalWire>(`/api/application-approvals/${approval.id}/validate`, {
...
async function consoleCommand(message: unknown, sender: chrome.runtime.MessageSender): Promise<unknown> {
  ...
  if (['pause-salary-backfill', 'cancel-salary-backfill'].includes(data.action || '')) {
    ...
    if (!stored || stored.runId !== data.runId) return { ok: false, error: '没有可操作的薪资回填计划。' }
  ...
  if (data.action === 'cancel') return requestRunnerCancel()
  if (data.action === 'resume') {
    if (pointer.autoMatch) return { ok: false, error: '含 AI 费用的旧任务请通过原扩展入口恢复；此入口仅免费采集。' }
    if (!pointer.pauseRequested || activeRunToken !== null) return { ok: false, error: '请等待任务确认暂停后再恢复。' }
    ...
    if (target.windowId !== source.windowId || !target.url || !isRunnerNavOrigin(target.url)) return { ok: false, error: '原 BOSS 标签页已关闭、移动或离开 BOSS，无法恢复。' }
    await chrome.tabs.update(pointer.tabId, { active: true })
    const foreground = await foregroundStartTab()
    if (!foreground.ok || foreground.tab.id !== pointer.tabId) return { ok: false, error: 'BOSS 标签页未处于前台。' }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (isConsoleUrl(sender.url)) return false // no access to the legacy popup protocol
  ...
  if (request.type === 'jobagent:detect-with-salary' && sender.tab?.id === undefined) {
    void (async () => {
      if (activeRunToken !== null || salaryOcrBusy) throw new Error('请先暂停自动任务，再进行单独检测。')
      const tabs = await chrome.tabs.query({ active: true, currentWindow: true })
      const tabId = tabs[0]?.id
      if (tabId === undefined) throw new Error('找不到当前标签页。')
      const response = await askTab<{ page_type: string; verification: boolean; candidates: SalaryCandidate[] }>(tabId, { type: 'jobagent:detect' })
      if (!response.ok || !response.result) throw new Error('页面未就绪，请刷新 BOSS 页面。')
  ...
  if (request.type === 'jobagent:runner-start') {
    const payload = message as { taskId?: number; candidateCap?: unknown; popupTarget?: unknown;
      matchApproval?: { confirmed: boolean; cap: number; fingerprint: string } }
  ...
  if (request.type === 'jobagent:runner-resume') {
    const payload = message as { popupTarget?: unknown }
    void (async () => {
      const binding = payload.popupTarget === undefined ? undefined : await bindPopupTarget(payload.popupTarget, sender)
