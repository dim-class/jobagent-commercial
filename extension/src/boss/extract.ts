...
var BossExtract = (function () {
  ...
  type ApplicationPreflight =
    ...
    | { status: 'login_required' | 'verification' | 'wrong_page' | 'identity_mismatch'
      ...
      //: BOSS's own words, never inferred from a missing control - it is the
      //: one status that retires the job rather than just refusing the attempt.
      | 'posting_closed' }
  ...
  function openCandidateLink(
    ...
    if (expectedUrl) {
      ...
      for (let i = 0; i < found.nodes.length; i += 1) {
        ...
        if (url && url === wanted) matches.push(i)
      }
      if (matches.length === 0) return { ok: false, error: 'card_gone' }
      if (matches.length > 1) return { ok: false, error: 'card_ambiguous' }
      index = matches[0]
    }
   ...
   */
  function postingClosed(doc: Document): boolean {
    ...
    return nodes.some((node) => {
      if (node.children.length) return false // leaf nodes only
  ...
  function preflightConfirmedApplication(
    ...
    if (postingClosed(doc)) return { status: 'posting_closed' }
  ...
  function greetingComposer(doc: Document, allowText = ''): {
    status: 'ok' | 'no_composer' | 'ambiguous_composer' | 'no_send_control' | 'input_not_empty'
    input?: HTMLElement
    send?: HTMLElement
  } {
    const view = doc.defaultView
    if (!view) return { status: 'no_composer' }
    ...
    const inputs = Array.from(doc.querySelectorAll(BossSelectors.GREETING_INPUT.join(',')))
      .filter((node): node is HTMLElement => node instanceof view.HTMLElement)
      .filter(visible)
    ...
    if (!inputs.length) return { status: 'no_composer' }
    if (inputs.length > 1) return { status: 'ambiguous_composer' }

    const input = inputs[0]
    const held = composerText(input).trim()
    if (held && held !== allowText) return { status: 'input_not_empty' }
   ...
   */
  function greetingDiagnostic(doc: Document): string {
    const view = doc.defaultView
    const visible = (node: Element): boolean => {
      const rect = node.getBoundingClientRect()
    ...
    const frames = Array.from(doc.querySelectorAll('iframe')).map((frame) => {
      ...
      try { return src ? new URL(src, doc.location.href).host : 'srcless' } catch { return 'bad' }
    })
    const wanted = BossSelectors.GREETING_SEND_TEXT
   ...
   */
  function paneChain(doc: Document): string {
  ...
  function isChatPage(currentUrl: string): boolean {
    try {
   ...
   */
  function chatConversationMatches(
    doc: Document,
    expectedCompany: string,
    expectedTitle: string,
  ): { status: string } {
    if (!expectedCompany || !expectedTitle) return { status: 'chat_job_unknown' }
    const panes = pickAll(doc, BossSelectors.CHAT_CONVERSATION)
    if (panes.nodes.length > 1) return { status: 'chat_job_ambiguous' }
  ...
  function greetingPageGate(
    ...
    if (!expectedExternalId) return 'wrong_job'
    const observedUrl = cleanUrl(currentUrl)
    const observedId = observedUrl ? externalIdOf(observedUrl) : null
    if (observedId) return observedId === expectedExternalId ? null : 'wrong_job'
    if (!isChatPage(currentUrl)) return 'left_job_page'
  ...
  function greetingComposerHolding(doc: Document, body: string): {
    status: string
    input?: HTMLElement
    send?: HTMLElement
  } {
    const composer = greetingComposer(doc, body)
    if (composer.status !== 'ok' || !composer.input) return composer
    return composerText(composer.input).trim() === body
      ? composer
      : { status: 'input_rejected' }
  }

  function sendConfirmedGreeting(
    doc: Document,
    greeting: string,
    currentUrl: string,
    expectedExternalId: string,
    ...
    if (!body) return { status: 'empty_greeting' }
    if (!expectedExternalId) return { status: 'wrong_job' }
    ...
    const gate = greetingPageGate(
      doc, currentUrl, expectedExternalId, expectedCompany, expectedTitle,
    )
    ...
    if (composer.input instanceof view.HTMLTextAreaElement) {
      ...
      if (setter) setter.call(composer.input, body)
      else composer.input.value = body
    } else {
    ...
    if (composerText(composer.input).trim() !== body) return { status: 'input_rejected' }
   ...
   * longer matches.
   */
  function submitConfirmedGreeting(
    doc: Document,
    greeting: string,
    currentUrl: string,
    expectedExternalId: string,
    ...
    const latch = view as Window & { __jobagentGreetingSent?: string }
    if (latch.__jobagentGreetingSent === body) return { status: 'already_sent' }
  ...
  function codeOf(el: Element, spec: FilterMenuSpec): string | null {
