...
var BossExtract = (function () {
   ...
   * not an answer.
   */
  function chatConversationJob(doc: Document): { status: string; externalId?: string } {
    ...
    if (!ids.size) return { status: 'chat_job_unknown' }
    if (ids.size > 1) return { status: 'chat_job_ambiguous' }
  ...
  function sendConfirmedGreeting(
    ...
    if (observedId) {
      ...
      if (owner.externalId !== expectedExternalId) return { status: 'chat_wrong_job' }
    } else {
