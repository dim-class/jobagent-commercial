...
var BossExtract = (function () {
   ...
   */
  function greetingComposer(doc: Document): {
    ...
    const inputs = Array.from(doc.querySelectorAll(BossSelectors.GREETING_INPUT.join(',')))
      .filter((node): node is HTMLTextAreaElement => node instanceof view.HTMLTextAreaElement)
      .filter(visible)
    ...
    if (input.value.trim()) return { status: 'input_not_empty' }
  ...
  function sendConfirmedGreeting(
    ...
    if (observedId) {
      if (observedId !== expectedExternalId) return { status: 'wrong_job' }
    } else if (isChatPage(currentUrl)) {
      const owner = chatConversationMatches(doc, expectedCompany, expectedTitle)
      if (owner.status !== 'ok') return { status: owner.status }
    } else {
    ...
    // nothing is sent.
    if (composer.input.value.trim() !== body) return { status: 'input_rejected' }
    composer.send.click()
    return { status: 'sent' }
  }
