...
;(function () {
  ...
  const detectBtn = $('detect') as HTMLButtonElement
  const previewBtn = $('preview') as HTMLButtonElement
  const devToggle = $('dev-mode') as HTMLInputElement
  const diagnosticPanel = $('diagnostic-panel')
  const diagnoseBtn = $('diagnose') as HTMLButtonElement
  const diagnosticCopyBtn = $('diagnostic-copy') as HTMLButtonElement
  const diagnosticOutput = $('diagnostic-output') as HTMLPreElement
  ...
  function renderCandidate(candidate: JobCandidate, index: number): HTMLElement {
    ...
    for (const field of FIELD_ORDER) {
      const raw = (candidate as unknown as Record<string, string | null>)[field]
      ...
      if (devMode()) {
        ...
        const hint = el(
          'div',
          'selector',
          selector ? `匹配选择器：${selector}` : '没有任何候选选择器命中',
        )
    ...
    if (devMode()) {
      const extras = Object.keys(candidate.matched_selectors).filter(
        (key) => FIELD_ORDER.indexOf(key) === -1,
      )
      if (extras.length) {
        card.appendChild(
          el(
            'div',
            'selector',
            extras.map((key) => `${key} ← ${candidate.matched_selectors[key]}`).join(' · '),
          ),
        )
      }
    }
  ...
  function render() {
    ...
    summary.appendChild(
      el('h3', undefined, PAGE_TYPE_LABEL[detection.page_type] || detection.page_type),
    )
    summary.appendChild(el('div', 'sub mono', detection.url || '（无 URL）'))
    summary.appendChild(
      el('div', 'count', `识别到 ${detection.candidates.length} 个岗位`),
    )
  ...
  function renderPreview(response: PreviewResponse) {
    ...
    for (const warning of response.warnings) card.appendChild(el('div', 'warn', warning))
    ...
    for (const row of response.rows) {
      ...
      if (row.status === 'new') {
        const button = el('button', 'btn-sm') as HTMLButtonElement
      ...
      for (const warning of row.warnings) line.appendChild(el('div', 'warn', warning))
   ...
   */
  async function callContentScript<T>(tabId: number, request: { type: string }): Promise<T> {
    try {
      ...
      if (response && response.ok && response.result !== undefined) return response.result
      ...
      const retry = (await chrome.tabs.sendMessage(tabId, request)) as {
        ok: boolean
        result?: T
        error?: string
      }
      if (retry && retry.ok && retry.result !== undefined) return retry.result
  ...
  async function detect() {
    ...
    try {
      ...
      if (detection.errors.length) setStatus('检测完成，但有问题。', 'warn')
      else if (!detection.candidates.length) setStatus('页面上没有找到岗位。', 'warn')
      else setStatus(`检测完成：${detection.candidates.length} 个岗位。`, 'ok')
    } catch (err) {
      setStatus('检测失败：' + String((err as Error)?.message || err), 'error')
    }
  }
  ...
  async function diagnose() {
    ...
    try {
      ...
      const result = await callContentScript<DiagnosticResult>(tab.id, {
        type: 'jobagent:diagnose',
      })
      diagnosticOutput.textContent = JSON.stringify(result, null, 2)
      diagnosticCopyBtn.disabled = false

      if (result.errors.length) setStatus('结构诊断完成，但有问题。', 'warn')
      else if (result.truncated) setStatus('结构诊断完成（已达到节点上限，结果已截断）。', 'warn')
      else setStatus('结构诊断完成，可点击「复制诊断结果」。', 'ok')
    } catch (err) {
      setStatus('结构诊断失败：' + String((err as Error)?.message || err), 'error')
    }
  }
  ...
  async function post(path: string, body: unknown): Promise<unknown> {
    ...
    if (!response.ok) {
      const message =
        (payload as { message?: string })?.message || `请求失败（HTTP ${response.status}）`
  ...
  async function preview() {
    ...
    try {
      ...
      setStatus(
        `预览完成：新 ${response.new_count} / 重复 ${response.duplicate_count}。什么都还没有保存。`,
        'ok',
      )
    } catch (err) {
      setStatus(
        '无法连接本地 JobAgent：' +
          ...
          '（后端需要在 127.0.0.1:8000 运行）',
        'error',
      )
    }
  }

  async function importOne(index: number, button: HTMLButtonElement) {
    if (!detection) return
